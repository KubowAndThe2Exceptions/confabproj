...
csv.each do |row|
  User.create!(:name => row['name'], :email => row['email'], :admin => row['admin'], :encrypted_password => row['password'], 
  :event => row['event'], :venue => row['venue'], :press => row['press'], :vendor => row['vendor'], :activated => row['activated'], 
  :activated_at => row['activated_at'], :zip => row['zip'], :address => row['address'], :describe => row['describe'])
end
